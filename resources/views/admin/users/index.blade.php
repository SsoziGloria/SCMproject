            ...
            <div class="card-body">
                <form id="bulk-form" action="{{ route('admin.users.bulk-action') }}" method="POST">
                                                            ...
                                                            <td>
                                                                <div class="form-check">
                                                                    <input class="form-check-input user-checkbox" type="checkbox" name="user_ids[]"
                                                                        value="{{ $user->id }}" {{ auth()->id() == $user->id ? 'disabled' : '' }}>
                                                                </div>
                                                            </td>
                                                            <td>
                                                                    ...
                                                                    <div class="me-3">
                                                                        @if($user->profile_photo)
                                                                            <img src="{{ asset('storage/' . $user->profile_photo) }}"
                                                                                class="rounded-circle" alt="{{ $user->name }}" width="40" height="40"
                                                            ...
                                                            <td>
                                                                <div class="form-check form-switch">
                                                                    <input class="form-check-input status-switch" type="checkbox"
                                                                        data-user-id="{{ $user->id }}" {{ $user->is_active ? 'checked' : '' }} {{ auth()->id() == $user->id ? 'disabled' : '' }}>
                                                                </div>
                                                            </td>
                    ...
                    <div class="d-flex justify-content-between align-items-center mt-4">
                        <div>
                            <span class="text-muted">Showing {{ $users->firstItem() ?? 0 }} to {{ $users->lastItem() ?? 0 }}
                                of {{ $users->total() }} users</span>
                        </div>
                        <div>
                            {{ $users->links() }}
                        </div>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            document.querySelectorAll('.status-switch').forEach(function (switchElem) {
                switchElem.addEventListener('change', function () {
                    ...
                    const switchElement = this; // Store reference for use in callbacks
                    const row = switchElement.closest('tr'); // Get the parent row

                    // Visual feedback immediately - add opacity to show processing
                    ...
                    fetch(`{{ url('admin/users') }}/${userId}/toggle-status`, {
                        ...
                        .then(data => {
                            ...
                            if (data.success) {
                                ...
                                if (statusBadge) {
                                    ...
                                    statusBadge.className = `badge bg-${isChecked ? 'success' : 'secondary'} status-badge`;
                                }

                                // Flash the row to indicate successful update
                                ...
                                // Update toast style based on action
                                toastEl.classList.remove('bg-success', 'bg-danger');
                                toastEl.classList.add(isChecked ? 'bg-success' : 'bg-danger');

                                // Set message
                                toastBody.textContent = data.message || `User ${isChecked ? 'activated' : 'deactivated'} successfully`;
                                toast.show();

                            } else {
                                // Revert switch if server reports failure
                        ...
                        .catch(error => {
                            ...
                            // Reset visual state and revert the switch
                            row.style.opacity = '1';
                            switchElement.disabled = false;
                            switchElement.checked = !isChecked;
