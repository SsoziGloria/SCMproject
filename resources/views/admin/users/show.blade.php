            ...
            <div class="card shadow-sm mb-4">
                ...
                <div class="card-body">
                    <ul class="list-group list-group-flush">
                        ...
                        {{-- <li class="list-group-item d-flex justify-content-between align-items-center">
                            Last Login
                            <span>{{ $user->last_login_at ? $user->last_login_at->format('M d, Y H:i') : 'Never'
                                }}</span>
                        </li> --}}
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            Orders
                        ...
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            Total Spent
                            <span>UGX {{ number_format($totalSpent ?? 0, 0) }}</span>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            Account Status
                    ...
                    <div class="tab-content" id="userTabsContent">
                        ...
                        <div class="tab-pane fade" id="activities" role="tabpanel" aria-labelledby="activities-tab">
                            ...
                            <div class="text-center py-5">
                                ...
                                <p class="mt-3 text-muted">No activity has been recorded for this user yet.</p>
                            </div>
                            @else
                            <ul class="timeline">
                                @foreach($activities as $activity)
                                @php
                                // Set icon and color based on the event description
                                $icon = 'bi-activity'; // Default
                                $color = 'secondary';
                                if (str_contains($activity->description, 'created')) {
                                $icon = 'bi-plus-circle'; $color = 'success';
                                } elseif (str_contains($activity->description, 'updated')) {
                                $icon = 'bi-pencil-square'; $color = 'info';
                                ...
                                <li class="timeline-item mb-4">
                                    <div class="timeline-marker bg-{{ $color }}">
                                        <i class="bi {{ $icon }} text-white"
                                            style="font-size: 0.5rem; line-height: 1;"></i>
                                    </div>
                                    <div class="timeline-content">
                                        <p class="mb-1">
                                            {{-- This will say "Product has been updated" etc. --}}
                                            <strong>{{ $activity->description }}</strong>

                                            {{-- If the activity has a subject, link to it --}}
                                            @if($activity->subject)
                                            <a href="#">{{ $activity->subject->name ??
                                                class_basename($activity->subject_type) }}</a>
                                            @endif
                                        </p>

                                        {{-- Display the changed data if it exists --}}
                                        @if($activity->properties->has('old'))
                                        <div class="text-muted small">
                                            @foreach($activity->properties['old'] as $key => $value)
                                            <div>
                                                Changed <strong>{{ $key }}</strong> from '{{ $value }}' to '{{
                                        ...
                                        <small class="text-muted">{{ $activity->created_at->diffForHumans() }} ({{
                                            $activity->created_at->format('M d, Y H:i') }})</small>
                                    </div>
                                </li>
                                @endforeach
                            </ul>
