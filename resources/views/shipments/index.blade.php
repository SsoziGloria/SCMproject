...
<div class="container-fluid py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <div>
            ...
            @endif
            <button type="button" class="btn btn-outline-success ms-2" data-bs-toggle="modal" data-bs-target="#exportModal">
    ...
    <div class="row mb-4">
        ...
        <div class="col-xl-3 col-md-6">
            <div class="card bg-warning text-white mb-4">
                ...
                <div class="card-footer d-flex align-items-center justify-content-between">
                    <a class="small text-white stretched-link" href="{{ route('shipments.index', ['status' => 'pending']) }}">View Details</a>
                    <div class="small text-white"><i class="bi bi-arrow-right"></i></div>
                </div>
            </div>
        </div>
        ...
        <div class="col-xl-3 col-md-6">
            <div class="card bg-info text-white mb-4">
                ...
                <div class="card-footer d-flex align-items-center justify-content-between">
                    <a class="small text-white stretched-link" href="{{ route('shipments.index', ['status' => 'shipped']) }}">View Details</a>
                    <div class="small text-white"><i class="bi bi-arrow-right"></i></div>
                </div>
            </div>
        </div>
            ...
            <div class="card {{ $stats['overdue'] > 0 ? 'bg-danger' : 'bg-success' }} text-white mb-4">
                <div class="card-body d-flex align-items-center justify-content-between">
                    <div>
                        ...
                        <div class="h5 mb-0 font-weight-bold">{{ $stats['overdue'] > 0 ? $stats['overdue'] : $stats['delivered'] }}</div>
                    </div>
                    <div>
                        <i class="bi {{ $stats['overdue'] > 0 ? 'bi-exclamation-triangle' : 'bi-check-circle' }} fs-1"></i>
                    </div>
                </div>
                <div class="card-footer d-flex align-items-center justify-content-between">
                    <a class="small text-white stretched-link" href="{{ route('shipments.index', ['status' => $stats['overdue'] > 0 ? 'pending' : 'delivered']) }}">View Details</a>
                    <div class="small text-white"><i class="bi bi-arrow-right"></i></div>
                </div>
            </div>
        </div>
                ...
                <div class="row g-3">
                    <div class="col-md-3">
                        <div class="input-group">
                            ...
                            <input type="text" class="form-control" name="search" value="{{ request('search') }}" placeholder="Shipment #, Product...">
                        </div>
                    </div>
                                ...
                                <td>
                                    <div class="d-flex align-items-center">
                                        @if($shipment->product && $shipment->product->image)
                                            <img src="{{ asset('storage/' . $shipment->product->image) }}" alt="{{ $shipment->product->name }}" class="me-2" style="width: 40px; height: 40px; object-fit: cover;">
                                        @else
                                    ...
                                    <div class="btn-group">
                                        ...
                                        @if(Auth::user()->role !== 'supplier' || Auth::user()->id === $shipment->supplier_id)
                                            @if($shipment->canBeShipped() && $shipment->status === 'pending')
                                                <form action="{{ route('shipments.update-status', $shipment) }}" method="POST" class="d-inline">
                                                    @csrf
                                                    @method('PUT')
                                                    <input type="hidden" name="status" value="shipped">
                                                    <button type="submit" class="btn btn-sm btn-outline-primary" title="Mark as Shipped">
                                            ...
                                            @if($shipment->canBeDelivered() && $shipment->status === 'shipped')
                                                <form action="{{ route('shipments.update-status', $shipment) }}" method="POST" class="d-inline">
                                                    ...
                                                    <button type="submit" class="btn btn-sm btn-outline-success" title="Mark as Delivered">
                            ...
                            <tr>
                                <td colspan="8" class="text-center py-5">
                                    <div class="d-flex flex-column align-items-center">
                                        ...
                                        <p class="text-muted">{{ request()->has('search') ? 'Try a different search term or filter' : 'Create your first shipment to get started' }}</p>
                                        @if(Auth::user()->role !== 'supplier')
