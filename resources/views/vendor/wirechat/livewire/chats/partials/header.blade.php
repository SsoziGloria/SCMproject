...
<header class="px-3 z-10 sticky top-0 w-full py-2 " dusk="header">
            ...
            @if ($showNewChatModalButton)
                ...
                <x-wirechat::actions.new-chat widget="{{$this->isWidget()}}">
                    <button id="open-new-chat-modal-button" class=" flex items-center focus:outline-hidden">
                        ...
                        {{-- <svg
                            class="w-7 h-7 -mb-1 stroke-[0.1]  stroke-none text-gray-500 hover:text-gray-900 dark:hover:text-gray-200 dark:text-gray-300"
                            stroke="currentColor" viewBox="0 0 24 24" height="24" width="24"
                        ...
                        {{-- <svg
                            class="w-7 h-7 -mb-1 text-gray-500 hover:text-gray-900 dark:hover:text-gray-200 dark:text-gray-300"
            ...
            @if ($showHomeRouteButton)
                <a id="redirect-button" href="{{ config('wirechat.home_route', '/') }}" class="flex items-center">
                    {{-- <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor"
                        class="bi bi-x-octagon-fill w-6 h-6 text-gray-500 dark:text-gray-400 transition-colors duration-300 dark:hover:text-gray-500 hover:text-gray-900"
                        viewBox="0 0 16 16">
                        <path
                    ...
                    {{-- <svg
                        class="bi bi-x-octagon-fill w-7 h-7 stroke-[0.5] text-gray-500 dark:text-gray-400 transition-colors duration-300 dark:hover:text-gray-500 hover:text-gray-900"
    ...
    @if ($allowChatsSearch)
        <section class="mt-4">
            <div
                class="px-2 rounded-lg dark:bg-[var(--wc-dark-secondary)]  bg-[var(--wc-light-secondary)]  grid grid-cols-12 items-center">
                ...
                <input id="chats-search-field" name="chats_search" maxlength="100" type="search"
                    wire:model.live.debounce='search' placeholder="{{ __('wirechat::chats.inputs.search.placeholder')  }}"
                    autocomplete="off"
