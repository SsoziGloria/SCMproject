    ...
    @foreach ($conversations as $key => $conversation)
        ...
        <li x-data="{
            ...
            <a @if ($widget) tabindex="0" role="button" dusk="openChatWidgetButton"
                ...
                @keydown.enter="$dispatch('open-chat',{conversation:@js($conversation->id)})" @else wire:navigate
            href="{{ route(WireChat::viewRouteName(), $conversation->id) }}" @endif
                ...
                class="py-2 flex gap-4 dark:hover:bg-[var(--wc-dark-secondary)]  hover:bg-[var(--wc-light-secondary)]  rounded-xs transition-colors duration-150  relative w-full cursor-pointer px-1"
                ...
                <aside class="grid  grid-cols-12 w-full">
                    ...
                    @if ($lastMessage != null && !$lastMessage?->ownedBy($this->auth) && !$isReadByAuth)
                        <div x-show="showUnreadStatus" dusk="unreadMessagesDot"
                            ...
                            <svg @style(['color:var(--wc-brand-primary)']) xmlns="http://www.w3.org/2000/svg" width="16"
                                height="16" fill="currentColor" class="bi bi-dot w-10 h-10 text-blue-500" viewBox="0 0 16 16">
