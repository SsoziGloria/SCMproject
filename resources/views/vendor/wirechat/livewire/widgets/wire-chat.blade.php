<div class="h-full ">
    ...
    <div x-data="{
        ...
        <div :class="chatIsOpen && 'hidden md:grid'"
            class="relative  w-full h-full sm:border-r border-[var(--wc-light-border)] dark:border-[var(--wc-dark-border)]    md:w-[360px] lg:w-[400px] xl:w-[350px] shrink-0 overflow-y-auto  ">
            <livewire:wirechat.chats :widget="true" />
        </div>
        <main x-data="ChatWidget()" x-on:open-chat.window="$wire.selectedConversationId= $event.detail.conversation;"
            ...
            :class="!chatIsOpen && 'hidden md:grid'" style="contain:content;">
            <div x-cloak x-show="show && showActiveComponent" x-transition:enter="ease-out duration-100"
                x-transition:enter-start="opacity-0 -translate-x-full"
                x-transition:enter-end="opacity-100 translate-x-0" x-transition:leave="ease-in duration-100 "
                x-transition:leave-start="opacity-100 translate-x-0"
                x-transition:leave-end="opacity-0 -translate-x-full" class="fixed inset-0" id="chatwidget-container"
                aria-modal="true">
                @forelse($widgetComponents as $id => $component)
                    <div x-show.immediate="activeWidgetComponent == '{{ $id }}'" x-ref="{{ $id }}" wire:key="key-{{$id }}"
                        class="h-full">
            ...
            <div x-show="!show && !chatIsOpen " class="m-auto  justify-center flex gap-3 flex-col  items-center ">
                ...
                <h4
                    class="font-medium p-2 px-3 rounded-full font-semibold bg-[var(--wc-light-secondary)] dark:bg-[var(--wc-dark-secondary)] dark:text-white dark:font-normal">
