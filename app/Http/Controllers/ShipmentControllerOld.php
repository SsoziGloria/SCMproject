...
class ShipmentController extends Controller
{
    ...
    {
        // Determine shipment type (default to incoming for supplier shipments)
        $type = $request->get('type', 'incoming');

        // Base query with shipment type filter
        $query = Shipment::with(['supplier', 'product', 'order'])
        ...
        if ($type === 'incoming' && $request->filled('supplier_id')) {
            $query->where('supplier_id', $request->supplier_id);
        }

        if ($type === 'outgoing' && $request->filled('order_id')) {
        ...
        if ($request->filled('search')) {
            ...
            $query->where(function ($q) use ($search, $type) {
                ...
                if ($type === 'incoming') {
                    ...
                    $q->orWhereHas('order', function ($q) use ($search) {
        ...
        $user = Auth::user();
        if ($user->role === 'supplier' && $type === 'incoming') {
        ...
        $stats = [
            'total' => Shipment::where('shipment_type', $type)->count(),
            'pending' => Shipment::where('shipment_type', $type)->where('status', 'pending')->count(),
            'shipped' => Shipment::where('shipment_type', $type)->where('status', 'shipped')->count(),
            'delivered' => Shipment::where('shipment_type', $type)->where('status', 'delivered')->count(),
            'overdue' => Shipment::where('shipment_type', $type)
                ->where('status', '!=', 'delivered')
                ->where('expected_delivery', '<', now())
                ->count(),
        ];

        return view('shipments.index', compact('shipments', 'suppliers', 'orders', 'stats', 'type'));
    }
    ...
    /**
     * Create an outgoing shipment for a customer order
     */
    public function createOrderShipment(Order $order, array $trackingData = [])
    {
        $shipment = Shipment::create([
            ...
            'customer_address' => $order->shipping_address ?? $order->address,
            'shipped_at' => now(),
            'estimated_delivery' => $trackingData['estimated_delivery'] ?? now()->addDays(3),
    ...
    /**
     * Get customer shipments (outgoing)
     */
    public function customerShipments(Request $request)
    {
        $request->merge(['type' => 'outgoing']);
        return $this->index($request);
    }

    /**
     * Get supplier shipments (incoming)
     */
    public function supplierShipments(Request $request)
    {
        $request->merge(['type' => 'incoming']);
        return $this->index($request);
    }
}
